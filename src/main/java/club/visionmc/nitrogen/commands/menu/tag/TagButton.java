...
public class TagButton extends Button {
    ...
    public List<String> getDescription(Player player) {
        ...
        description.add(Chat.format("&aShows up as:"));
        description.add(Chat.format(tag.getPrefix()));
        description.add(" ");
        if(!player.hasPermission("nitrogen.tags." + tag.getId())) {
            description.add(Chat.format("&cYou don't have permission to use this tag."));
        }else {
            description.add((profile.getActiveTag() == null || !profile.getActiveTag().getId().equals(tag.getId().toLowerCase()) ? Chat.LIGHT_GREEN + "Select this tag." : Chat.LIGHT_RED + "Unselect this tag."));
        }
        description.add(Chat.format("&7&m") + Chat.LINE);
        return description;
    }
