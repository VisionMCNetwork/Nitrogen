...
public class BanCommand {
    ...
    @Command(names = {"ban"}, permission = "nitrogen.punishments.ban")
    public static void ban(CommandSender sender, @Param(name = "target") Profile profile, @Flag(value = {"a", "announce"}) boolean announce, @Flag(value = {"c", "clear"}) boolean clear, @Param(name = "reason", wildcard = true) String reason) {
        ...
        sender.sendMessage(ChatColor.GREEN + "You've banned " + Chat.format(Chat.format(profile.getHighestRank().getColor() + profile.getUsername())) + ChatColor.GREEN + " for " + ChatColor.YELLOW + reason + ChatColor.GREEN + ".");
