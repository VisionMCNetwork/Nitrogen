...
public class TempMuteCommand {
    ...
    @Command(names = {"tempmute", "tmute"}, permission = "nitrogen.punishments.tempmute")
    public static void tempmute(CommandSender sender, @Param(name = "target") Profile profile, @Param(name = "duration") String duration, @Flag(value = {"a", "announce"}) boolean announce, @Param(name = "reason", wildcard = true) String reason){
        ...
        if(toSend != null){
            toSend.sendMessage(Chat.LIGHT_RED + "You have been muted for " + punishment.getRemainingText() + ".");
        }
    }
