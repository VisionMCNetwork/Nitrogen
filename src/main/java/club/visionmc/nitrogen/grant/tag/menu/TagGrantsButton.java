 ...
 * @date 11/27/2022
 */
public class TagGrantsButton extends Button {

    private final ProfileHandler profileHandler = Nitrogen.getInstance().getProfileHandler();

    private final TagGrant grant;

    public TagGrantsButton(TagGrant grant){
        this.grant = grant;
    }
    ...
    public List<String> getDescription(Player player) {
        ...
        description.add(Chat.YELLOW + "Tag" + Chat.LIGHT_GRAY + ": " + Chat.RESET.toString() + Chat.format(grant.getTag().getDisplayName()));
        description.add(Chat.YELLOW + "Added By" + Chat.LIGHT_GRAY + ": " + Chat.RESET.toString() + (addedBy == null || grant.getAddedBy().toString().equalsIgnoreCase(Profile.consoleUUID.toString()) ? Chat.DARK_RED.toString() + Chat.BOLD + "Console" : Chat.format(addedBy.getHighestTag().getColor() + addedBy.getUsername())));
        ...
        if(grant.isActive()){
            ...
            if(grant.getRemovedBy() == null && grant.getDuration() != Long.MAX_VALUE){
                ...
                removedBy = (grant.getRemovedBy().toString().equalsIgnoreCase(Profile.getConsoleUUID().toString()) || grant.getRemovedBy() == null ? Chat.DARK_RED.toString() + Chat.BOLD + "Console" : profileHandler.getProfileOrCreate(grant.getRemovedBy()).getHighestTag().getColor() + profileHandler.getProfile(grant.getRemovedBy()).getUsername());
            }
            description.add(Chat.LIGHT_GRAY.toString() + Chat.STRIKETHROUGH + Chat.LINE);
